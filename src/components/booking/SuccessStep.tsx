...
export default function SuccessStep({ orderNumber, totalAmount, vendorName, eventDate, guestCount }: SuccessStepProps) {
    const formattedDate = eventDate
        ? format(new Date(eventDate), 'dd MMM yyyy', { locale: id })
        : '-';
    ...
    return (
        <div className="max-w-xl mx-auto text-center py-10">
            ...
            <div className="bg-background-light border border-border-light rounded-xl px-6 py-5 max-w-sm mx-auto mb-8">
                ...
                <p className="text-2xl font-bold text-accent tracking-wide font-mono">#{orderNumber}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 max-w-sm mx-auto mb-10 text-left">
                <div className="bg-white border border-border-light rounded-lg p-3">
                    ...
                    <p className="text-sm font-semibold text-text-main">{vendorName}</p>
                </div>
                <div className="bg-white border border-border-light rounded-lg p-3">
                    <p className="text-[11px] text-text-muted">Estimasi Total</p>
                    <p className="text-sm font-semibold text-primary">{formatRupiah(totalAmount)}</p>
                </div>
                <div className="bg-white border border-border-light rounded-lg p-3">
                    <p className="text-[11px] text-text-muted">Tanggal Acara</p>
                    <p className="text-sm font-semibold text-text-main">{formattedDate}</p>
                </div>
                <div className="bg-white border border-border-light rounded-lg p-3">
                    <p className="text-[11px] text-text-muted">Jumlah Tamu</p>
                    <p className="text-sm font-semibold text-text-main">{guestCount} orang</p>
                </div>
            </div>
